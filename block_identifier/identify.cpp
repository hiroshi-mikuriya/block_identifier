...
/*!
 ...
 @return 最も近い色
 */
Color getColor(cv::Vec3b bgr, std::vector<Color> const & colors)
{
    double len2 = 100000;
    Color dst;
...
/*!
 ...
 @param[in] colors 判別する色情報
 @return ブロック情報
 */
std::vector<BlockInfo> getBlockInfo(cv::Mat const & m, std::vector<cv::Point> const & points, std::vector<Color> const & colors)
{
    ...
    for (int i = 0; i < blockCount; ++i){
        ...
        info.color = getColor(info.ave, colors);
...
void identifyBlock(cv::Mat const & m, std::vector<Color> const & colors)
{
    ...
    auto const blockInfo = getBlockInfo(m, contour, colors);
